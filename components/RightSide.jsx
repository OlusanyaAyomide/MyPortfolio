...
export default function RightSide() {
  ...
  const navItems = navsections.map((item,key)=>{
    return(
      <motion.button className={`rotate-90 text-lg hover:text-xl hover:text-sundry transition-all duration-200 ${activeSection === item?"text-sundry":""}`} key={key} variants={sideAnimationChildren}>{item}</motion.button>
    )
  })
  return (
